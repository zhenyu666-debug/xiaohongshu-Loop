...
B:sidecar-restart    [ACTIVE 2026-07-24 07:41] **Python and frontend package layout confusion blocks restart.** uvicorn :8888 (pid 39520), vite :5173 (pid 32332), ssh-serveo tunnel (pid 32540) all died between 22:04 (Wed) and 07:41 (Fri). Sandbox-side restart failed three times: (1) `C:\Python313\python.exe` referenced from notes does not exist on this machine — only `C:\Python311\python.exe`; (2) `web/package.json` does not exist — frontend is single-package layout at repo root with root `package.json`; AGENTS.md is stale relative to disk. Needs-me: confirm correct launcher (which python? which `npm` script from which dir?). `W:sup` supervisor `.neko/health_check.py` exists but cannot fix what it cannot find.

B:test-medgraph-hang [ACTIVE 2026-07-24 09:43] **Pre-existing hang in `test_medgraph_sample_basic`** (and possibly `test_medgraph_graph_structure`/`test_medgraph_patient_detail`). Reproduces on clean tree without W:cli-1 changes. Reproduces via `pytest -v` (no `-q`) → 95s+ no output past pytest header. Module-import collection works (1.04s). Likely cause: 2M-patient synthesis per W:med §3 W:med +2M default cap_and_default, OR a httpx call without timeout. Blocks full-suite CI gating — pytest must be invoked with explicit `--ignore=tests/test_medgraph.py`. Needs-me: open `fraud-risk-engine/tests/test_medgraph.py` and instrument with `pytest --forked` or skip-pattern until one offending test can be located.
