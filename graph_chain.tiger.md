...
B:gfw-push-active    [ACTIVE 2026-07-25 00:07] **Github 443 unreachable for 12+ minutes straight.** 8 retry attempts at 59-110s backoff all failed with "Failed to connect to github.com:443 after 21s". Worse than the 2026-07-21 §B:gfw-push pattern (which said windows were reachable). Local commit `3aef806` (fix medgraph hang) is on `main` but **not pushed**. Pattern: heartbeat may need to keep retrying until a window opens. Needs-me: wait for GFW window to reopen, then push. Workaround: alternative git remote (ssh://git@github.com:443 vs https), or git via gh-cli proxy.
