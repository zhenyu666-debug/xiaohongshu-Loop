...
# Application icon (multi-size .ico). Generated by scripts/build_icon.py.
# PyInstaller embeds this both into the EXE metadata and as the default
# icon for any windows it shows.
APP_ICON = REPO_ROOT / "assets" / "ico" / f"{APP_NAME}.ico"
...
def build() -> int:
    ...
    # Embed the application icon if the .ico file exists. Generated by
    # `scripts/build_icon.py` from the marketing PNG.
    if APP_ICON.exists():
        cmd += ["--icon", str(APP_ICON)]
        print(f"[build] embedding icon: {APP_ICON}")
    else:
        print(f"[build] no icon at {APP_ICON} (run scripts/build_icon.py to generate it)")
