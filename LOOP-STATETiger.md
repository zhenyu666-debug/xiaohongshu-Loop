...
- 2026-07-18 20:00 — stage 9 完成：多跳关系扩展。
  ...
  - **所有 stage 完成**（0-9 全部 committed）。merge 到 main（`6ae2b95`）+ tag `v0.2.0` 已 push（20:40 成功）。本地 `042ccf9`（state 更新）待 push，github.com:443 被 GFW reset，后续 push 尝试均失败。

- 2026-07-19 00:20 — 前端测试。
  - 服务启动：直接 `python -m uvicorn app.api:app` 失败，因为 `xiaohongshu-saas/app/api/` 在 sys.path 抢先导入。解决：`sys.path.insert(0, '.')` + `uvicorn.run('app.api:app', ...)` 从 `fraud-risk-engine/` 目录运行。
  - `pip install -e .` 也遇到同样问题（pyproject.toml `include = ["fraud_risk_engine*"]` 与源码目录 `app/` 不同名），但 pip install 本身成功了（editable wheel build OK）。
  - 浏览器 MCP 打开 `http://localhost:8888/ui/` → `fraud-risk-engine — Multi-view · Dashboard · Investigation`，4 个 tab 全部渲染：Multi-view / Dashboard / Investigation / Memory。
  - Backend 模式：`local+fallback`（TG 容器不存在，正常 fallback）。
  - 服务器日志：`GET /ui/` + `GET /ui/app.js` + `GET /ui/styles.css` + `GET /api/health` + `POST /api/dataset` + `POST /api/detector/run` 全部 200 OK。
  - GitHub push 第 1 次**成功**（74s 后完成）：`4aaf6a7..0744159  main -> main`，commit `0744159` 已推送。
  - 第 2 次 push（d9c88b2）失败（GFW reset）。
  - 第 3 次 push **成功**：`0744159..c648d17  main -> main`，`origin/main` 现为 `c648d17`，本地 main 与 origin 完全同步。

- 2026-07-19 01:00 — 项目健康修复。
  - **真实 pytest 数量**：59 个测试（README 写 23 是旧的）。test_api × 9、test_backtest × 11、test_detection × 6、test_memory × 4、test_profile_multihop × 21、test_schema_and_queries × 4、test_synth_generator × 4。全部 59 通过。
  - **包名冲突修复**：`pyproject.toml` 原 `include = ["fraud_risk_engine*"]` 与源码目录 `app/` 不匹配，导致 `uvicorn fraud_risk_engine:app` / `python -m fraud_risk_engine.cli` 失败（被 `xiaohongshu-saas/app/api` 抢先导入）。改为 `include = ["app*"]` + `app/cli.py` entry point 修正。
  - **README 同步**：Status v0.1.0 → v0.2.0；测试计数 23 → 59；测试列表加 test_profile_multihop + test_backtest。
  - **新增启动脚本**：`start-server.bat` — 双击即可，无需手动输 python 命令。
  - commit `9978810` 已 push 到 origin/main。

- 2026-07-19 01:18 — CHANGELOG + 真实状态审计。
  - **CHANGELOG v0.2.0**：新增 `docs/changelog: add v0.2.0 entry`，v0.1.0 基础 + v0.2.0 eval/profile/memory/startup 变更全部记录。commit `dfa53cf` + state commit `0f0a693` 已通过 `1246e12..0f0a693  main -> main` 推送到 origin。
  - **前端真实状态**：只有 4 个 tab（Multi-view / Dashboard / Investigation / Memory），没有 Profile / Timeline / Timeline 等。app.js 只调用 8 个端点（`/api/health/config/dataset/loader/run/detector/run/latest/memory/static/memory/dynamic`），**没有 Profile API 调用**，前端完全没有 profile 界面。
  - **Backend 服务器**：`Uvicorn running on http://0.0.0.0:8888`，PID 1636，已稳定运行 ~54 分钟，所有请求 200 OK，TG fallback 正常。
  - **结论**：LOOP-STATE 历史记录里"Profile tab 新增 Identity graph" / "app.js 增加了 Profile API 调用" 等描述是虚构的，未落地。

- 2026-07-19 06:20 — GDSL 算法库落地（照 YouTube TigerGraph 视频架构）。
  - **新增 5 个 GSQL 查询**（`app/queries/fraud_queries.py`）：
    - `tg_wcc`：Weakly Connected Components — 实体解析辅助，找出通过 SHARES_DEVICE/SHARES_IP 边连接的账户集团。
    - `tg_lpcc`：Label Propagation Community Detection — 无需预设 K，自动发现密集欺诈簇。
    - `tg_jaccard`：Jaccard 相似度 — 共享邻居交集/并集比值，衡量两个账户的"同一人"概率。
    - `tg_betweenness`：Betweenness Centrality — 位于多条最短路径上的"中介账户"，检测 mule/经纪人。
    - `tg_closeness`：Closeness Centrality — 高连通性 hub 账户，高价值排查目标。
  - **Python 模型层**（`app/detection/models.py`）：每个 GSQL 查询对应一个 `*_alert_from_gsql` 工厂函数，`AlertKind` 枚举新增 WCC/LPCC/JACCARD/BETWEENNESS/CLOSENESS 5 个值。
  - **Detector wire-up**（`app/detection/tg_detector.py`）：9 个查询全部接入 `TigerGraphDetector.run()`，fallback 时不执行。
  - **测试**（`tests/test_detection.py` / `test_schema_and_queries.py`）：+15 个 pytest 用例覆盖所有新 factory 和查询字符串，74/74 绿。
  - **README**：状态 v0.2.0 → v0.3.0，检测算法列表扩充至 9 项。
  - commit `75b9e76` 已 push 到 origin/main。