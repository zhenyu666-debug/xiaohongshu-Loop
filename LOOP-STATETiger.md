...
## 当前阶段
stage 7 — MD 记忆体落地（4 helper 测试 + 2 API 测试，pytest 34/34 绿，commit 2dfc6cc）；stages 6, 9 待办
...
- [x] stage 5 — 评估回测（PR sweep + 阈值扫描 + HTML 报告）— `app/eval/backtest.py` + 11 测试，commit `cf24256` on `tiger/stage5-backtest-harness`
- [x] stage 7 — MD 记忆体（静态 + 动态，API `/api/memory/{static,dynamic}`，4 helper 测试 + 2 API 测试），commit `2dfc6cc` on `tiger/stage5-backtest-harness`
- [ ] stage 6 — 可视化三层（frontend/{index.html, app.js, styles.css} 4 tab 已在 working tree；尚未 `git add`）
- [ ] stage 8 — pytest 23/23（已确认 baseline） → 现 34/34
- [ ] stage 9 — 多跳关系扩展（identity graph + funds flow graph）
